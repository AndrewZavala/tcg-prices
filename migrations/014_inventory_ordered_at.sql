-- Explicit TCG order date for "Need to Sell" aging (backfill from acquired_at / created_at).

ALTER TABLE inventory_lots
    ADD COLUMN IF NOT EXISTS ordered_at DATE;

UPDATE inventory_lots
SET ordered_at = COALESCE(acquired_at, created_at::date)
WHERE ordered_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_inventory_lots_ordered_at
    ON inventory_lots (ordered_at)
    WHERE ordered_at IS NOT NULL AND qty_on_hand > 0;

-- Refresh view so ordered_at is visible via il.*.
